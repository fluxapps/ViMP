 ...
 */
class xvmpEditVideoFormGUI extends ilPropertyFormGUI {
	...
	public function __construct($parent_gui, $mid = 0) {
		global $ilCtrl, $lng;
		$this->lng = $lng;
		...
		$this->setTitle($this->video ? $this->pl->txt('edit_video') : $this->pl->txt('upload_video'));
...
//		if ($media_permissions )
	...
	protected function formatVideo() {
		foreach (array(array('categories', 'category', 'cid'), array('tags', 'tag', 'tid')) as $labels) {
			$array = array();
			if (count($this->video[$labels[0]][$labels[1]]) == 1) {
				$this->video[$labels[0]][$labels[1]] = array( $this->video[$labels[0]][$labels[1]] );
			}
			foreach ($this->video[$labels[0]][$labels[1]] as $cat) {
				$array[$cat[$labels[2]]] = $cat['name'];
			}
	...
	function setValuesByArray($a_values, $a_restrict_to_value_keys = false) {
		...
		parent::setValuesByArray($a_values, $a_restrict_to_value_keys); // TODO: Change the autogenerated stub
	}
}