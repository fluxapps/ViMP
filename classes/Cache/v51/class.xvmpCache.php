...
class xvmpCache extends ilGlobalCache {
	...
	public static function getCacheInstance() {
		...
		if (str_replace('.', '', ILIAS_VERSION_NUMERIC) >= 510) {
			//			$xvmpCache = parent::getInstance(self::COMP_OPENCAST);
			$xvmpCache = new self($service_type);
			$xvmpCache->initCachingService();
			$xvmpCache->setActive(true);
		} else {
			$xvmpCache = new self($service_type);
		}
		$xvmpCache->setActive(true);
		$xvmpCache->setOverrideActive(true);
